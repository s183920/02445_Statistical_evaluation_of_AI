...
for (i in c(1:K_fold)){
  ...
  for (k in K){
  pred_KNN <- knn(train_set[,-c(1,2)], test_set[,-c(1,2)], train_set$persons, k = k)
  loss_KNN <- mean(pred_KNN != test_set$persons)
  loss[k,i] <- loss_KNN
...
## ANOVA in each direction

```{r}
library(dplyr)

diff_data <-data_long %>% 
  group_by(experiment, person, repetition, direction) %>% 
  summarise(diff = max(value, na.rm = T) - min(value, na.rm = T)) %>% 
...
p_vals_BH <- p.adjust(p_vals_normal, method = "BH")
...
group_by(experiment) %>% filter(experiment == 1) %>% plot(x) 

diff_data$x

plot(diff_data["experiment" == 1,]$x)
```

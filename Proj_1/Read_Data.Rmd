...
for (i in c(1:K_fold)){
  ...
  fit <- multinom(data = data_KNN[train_index,], formula = Person ~ X + Y  + Z+ as.numeric(Time))
  predictions <- predict(fit, data_KNN[c(4:7)][test_index,])
  print(  mean(predictions == test_set$Person ) )
  for (k in K){
...
```{r}
library(nnet)
fit <- multinom(data = data_exp5, formula = Person ~ X + Y  + Z+ as.numeric(Time))
summary(fit)
predict(fit , head(data_exp5[,c(4:7)], 100))
...
```{r}
model_anova <- lm(data = data_long, formula = Y ~ Experiment)
summary(model_anova)
anova(model_anova)

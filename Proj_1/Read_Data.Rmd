...
data_wide
mean(sample(rownames,100) == rownames)
```
...
for (i in 1:N){
  ...
  train_index <- s[-i]
  test_index <- s[i]
  train_set <- data_wide[train_index,]
  test_set <- data_wide[test_index,]
  
  for (k in K){
    
    E <- sum(knn(train_set[colnames], test_set[colnames], train_set$rownames, k = k) != test_set$rownames)
    errors[i,k] <- E
